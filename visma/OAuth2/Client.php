...
class Client
{
    ...
    */
    ...
    {
        woovisma_addlog("get access token start");
        if (!$grant_type) {
            woovisma_addlog("The grant_type is mandatory");
        ...
        if (!class_exists($grantTypeClass)) {
            woovisma_addlog('Unknown grant type \'' . $grant_type . '\'');
        ...
        if (!defined($grantTypeClass . '::GRANT_TYPE')) {
            woovisma_addlog('Unknown constant GRANT_TYPE for class ' . $grantTypeClassName);
        ...
        woovisma_addlog("parameters:".print_r($parameters ,true));
        $http_headers = $extra_headers;
        woovisma_addlog("http headers:".print_r($http_headers ,true));
        woovisma_addlog("client auth:".print_r($this->client_auth ,true));
        ...
        woovisma_addlog("parameters before execute:".print_r($parameters ,true));
        woovisma_addlog("http headers before execute:".print_r($http_headers ,true));
    ...
    {
        woovisma_addlog("---------CURL Start---------");
        $curl_options = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYPEER => true,
            CURLOPT_CUSTOMREQUEST  => $http_method
        );
        woovisma_addlog("http method:".$http_method);
        switch($http_method) {
        ...
        woovisma_addlog("curl options:".print_r($curl_options,true));
        $ch = curl_init();
        ...
        if (!empty($this->curl_options)) {
            woovisma_addlog("curl options override:".print_r($this->curl_options,true));
            curl_setopt_array($ch, $this->curl_options);
        }
        $result = curl_exec($ch);
        woovisma_addlog("curl result fetched");
        $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        $content_type = curl_getinfo($ch, CURLINFO_CONTENT_TYPE);
        if ($curl_error = curl_error($ch)) {
            woovisma_addlog("Curl Error: ".print_r($curl_error,true));
            throw new Exception($curl_error, Exception::CURL_ERROR);
        } else {
            $json_decode = json_decode($result, true);
        }
        curl_close($ch);
        woovisma_addlog("---------CURL End---------");
