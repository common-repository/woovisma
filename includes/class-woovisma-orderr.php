...
class Woovisma_OrderR extends WooVismaClient
{
    ...
    public static function &getInstance($client_auth = self::AUTH_TYPE_URI, $certificate_file = null,$refresh=false)
    {
        ...
        if($refresh) return new Woovisma_OrderR($client_auth, $certificate_file);
        if(!isset($objInstance[$client_auth]))
        {
            $objInstance[$client_auth]=new Woovisma_OrderR($client_auth);
        }
    ...
    public function getOrders($modifiedTime=false)
    {
    ...
    public function setOrder(DSVismaOrder $order)
    {
        try
        {
            if(empty($order->Id))
                $response = $this->fetch($this->objConfig->apiEndpoint()."/v1/orders",$order->render(),OAuth2\Client::HTTP_METHOD_POST,array(), OAuth2\Client::HTTP_FORM_CONTENT_TYPE_APPLICATION);
            else
            {
                $response = $this->fetch($this->objConfig->apiEndpoint()."/v1/orders/".$order->Id,$order->render(),OAuth2\Client::HTTP_METHOD_PUT,array(), OAuth2\Client::HTTP_FORM_CONTENT_TYPE_APPLICATION);
            }
